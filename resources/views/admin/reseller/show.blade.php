    ...
    <div id="content" class="container-fluid">
        <div class="card">
            <div class="card-header font-weight-bold d-flex justify-content-between align-items-center">
                <h5 class="m-0 ">List reseller <a href="{{ url('admin/reseller/add') }}"
            ...
            <div class="card-body">
                ...
                @endif
                <form action="{{ url('admin/reseller/action') }}" method="GET">
                    <div class="analytic">
                        <a href="{{ url('admin/reseller') }}" class="text-primary">All<span
                                class="text-muted">({{ $countItem }})</span></a>
                        <a href="{{ url('admin/reseller?status=trash') }}" class="text-primary">
                    ...
                    <table class="table table-striped table-checkall">
                        ...
                        <tbody>
                            @php
                                $t = 0;
                            @endphp
                            @foreach ($list_reseller as $reseller)
                                @php
                                    $t++;
                                @endphp
                                <tr>
                                    <td>
                                        <input type="checkbox" name="checkItem[]" value="{{ $reseller->id }}">
                                    </td>
                                    <td scope="row">{{ $t }}</td>
                                    ...
                                    <td style="display: flex; justify-content: space-between;">
                                        <a href="{{ $reseller->url }}">{{ $reseller->url }}</a>
                                        <div class="action-wp">
                                            @if ($trash == true)
                                                <a href="{{ url('admin/reseller/restore/' . $reseller->id) }}"
                                            ...
                                            @endif
                                            <a href="{{ url('admin/reseller/edit/' . $reseller->id) }}" class="action"
                                                title="Edit">
                                                <svg xmlns="http://www.w3.org/2000/svg" height="20px"
                                                    viewBox="0 -960 960 960" width="20px" fill="#000">
                                                    <path
                                            ...
                                            @if ($trash == true)
                                                <a href="{{ url('admin/reseller/forceDelete/' . $reseller->id) }}"
                                            ...
                                            @else
                                                <a href="{{ url('admin/reseller/delete/' . $reseller->id) }}"
                                                    class="action" title="Delete" style="margin-right: 20px">
                                                    <svg xmlns="http://www.w3.org/2000/svg" height="20px"
                                                        viewBox="0 -960 960 960" width="20px" fill="#000">
                                                        <path
